...
async def human_amd_result(
    ...
    if is_machine:
        logger.warning(f"[{session_id}] AMD detected machine/voicemail - hanging up immediately")

        # Hang up the voicemail call immediately so we don't leave a message
        try:
            twilio_voice.client.calls(CallSid).update(status="completed")
            logger.info(f"[{session_id}] Voicemail call {CallSid} hung up")
        except Exception as e:
            logger.error(f"[{session_id}] Failed to hang up voicemail call: {e}")

        # Treat this as a declined/failed escalation
        await twilio_voice.handle_human_declined(session_id)
        logger.info(f"[{session_id}] Escalation marked as failed due to voicemail")
...
async def media_stream_websocket(websocket: WebSocket):
    ...
    try:
        async for message in websocket.iter_text():
            ...
            if event == "connected":
                ...
                if payload and stream_sid:
                    call = twilio_voice.get_call_by_stream(stream_sid)
                    if not call:
                        continue

                    was_speaking = call.is_speaking

                    # VAD only - fast, non-blocking (no STT/LLM/TTS here)
                    utterance_audio = twilio_voice.process_audio_chunk(stream_sid, payload)

                    # Barge-in: user just started speaking during playback/processing
                    if call.is_speaking and not was_speaking:
                        if call.is_playing_audio or (current_response_task and not current_response_task.done()):
                            logger.info(f"[{session_id}] BARGE-IN: User speaking during playback/processing")
                            # Cancel current processing/sending task
                            if current_response_task and not current_response_task.done():
                                current_response_task.cancel()
                            # Signal barge-in to stop audio sending
                            call.barge_in_requested = True
                            # Tell Twilio to clear buffered audio immediately
                            try:
                                if websocket.client_state == WebSocketState.CONNECTED:
                    ...
                    # Complete utterance detected - process in background task
                    if utterance_audio is not None:
                        # Cancel any previous processing task
                        if current_response_task and not current_response_task.done():
                            current_response_task.cancel()
                        # Process STT -> LLM -> TTS -> send audio in background
                        current_response_task = asyncio.create_task(
                            process_and_respond(websocket, stream_sid, session_id, utterance_audio)
                        )
        ...
        # Cancel background response task if running
...
async def send_audio_to_stream(websocket: WebSocket, stream_sid: str, audio_data: bytes, session_id: str = None) -> bool:
    ...
    try:
        ...
        try:
            ...
            # Always reset playing state (handles normal exit, barge-in, and task cancellation)
            if session_id:
...
async def process_and_respond(websocket: WebSocket, stream_sid: str, session_id: str, utterance_audio: bytes):
    """
    Process a complete utterance in the background: STT -> LLM -> TTS -> send audio.
    Runs as an asyncio task so the WebSocket loop stays responsive for barge-in.
    """
    call = twilio_voice.get_call_by_session(session_id)
    if not call:
        return
    ...
    try:
        # Process utterance (STT + LLM + TTS)
        response_audio = await twilio_voice._process_utterance(call, utterance_audio)
        ...
        logger.info(f"[{session_id}] Response cancelled (barge-in)")
        raise
    except Exception as e:
        logger.error(f"[{session_id}] Error processing utterance: {e}")
        try:
            await send_error_audio(websocket, stream_sid, session_id)
        except Exception:
            pass
    finally:
        # Ensure state is reset if we were cancelled mid-processing
        if call and call.state == CallState.PROCESSING:
            call.state = CallState.AI_CONVERSATION
