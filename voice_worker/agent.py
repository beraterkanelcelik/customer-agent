...
class DealershipVoiceAgent:
    ...
    async def exit_idle_mode(self, speak_resume: bool = True):
        ...
        # Let agent generate resume message - no hardcoded message
        if speak_resume:
            try:
                response = await self.http_client.post(
                    ...
                    json={
                        "session_id": self.session_id,
                        "message": "[ESCALATION_RETURNED:human_ended]"  # Special marker
                    }
                )
                response.raise_for_status()
                data = response.json()
                resume_text = data.get("response", "")
                if resume_text:
                    await self.speak(resume_text)
            except Exception as e:
                logger.error(f"Failed to get resume message from agent: {e}")
    ...
    async def _speak_human_introduction(self, human_id: str):
        ...
        Let the agent generate the message - no hardcoded template.
        """
        logger.info(f"Getting introduction for human {human_id}")

        # Temporarily exit idle mode to speak (just for this message)
        was_idle = self.is_idle
        self.is_idle = False
        try:
            # Let agent generate introduction message
            response = await self.http_client.post(
                "/api/chat",
                json={
                    "session_id": self.session_id,
                    "message": f"[HUMAN_JOINED:{human_id}]"  # Special marker
                }
            )
            response.raise_for_status()
            data = response.json()
            intro_message = data.get("response", "")
            if intro_message:
                logger.info(f"Speaking intro to human {human_id}: {intro_message}")
                await self.speak(intro_message)
        except Exception as e:
            logger.error(f"Failed to get human introduction from agent: {e}")
        finally:
    ...
    async def run(self, ctx):
        ...
        # Send greeting - generated by agent via API
        try:
            response = await self.http_client.post(
                "/api/chat",
                json={
                    "session_id": self.session_id,
                    "message": "[CALL_STARTED]"  # Special marker for initial greeting
                }
            )
            response.raise_for_status()
            data = response.json()
            greeting = data.get("response", "")
            if greeting:
                await self.speak(greeting)
        except Exception as e:
            logger.error(f"Failed to get greeting from agent: {e}")
                    ...
                    while self._running:
                        try:
                            ...
                            # Handle end call signal
                                ...
                                farewell = data.get("farewell_message", "")  # Agent must provide farewell
                                logger.info(f"Received end_call signal: {farewell[:50] if farewell else '(no farewell)'}...")
    ...
    async def _handle_user_speech(self, audio_data: bytes, sample_rate: int):
        ...
        try:
            ...
            try:
                error_response = await self.http_client.post(
                    "/api/chat",
                    json={
                        "session_id": self.session_id,
                        "message": "[PROCESSING_ERROR]"  # Special marker for error
                    }
                )
                error_response.raise_for_status()
                error_data = error_response.json()
                error_text = error_data.get("response", "")
                if error_text:
                    await self.speak(error_text)
            except Exception:
